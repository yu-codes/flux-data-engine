...
| **Platform** | **Workspace（多租戶隔離，資源以 workspace 為界）**、**Auth（JWT）+ RBAC（admin / editor / viewer）**、**Audit 稽核軌跡**、**Schedules（interval 或 cron；可排程 Model 或 Pipeline）**、**Job（佇列／lease／heartbeat／取消／重試上限）+ SSE 進度串流**、**Prometheus metrics（含 `flux_execution_duration_seconds{provider,kind,status}`）**、request correlation id（寫進每一行 log） |
...
十一個 Provider 中只有一個可訓練 —— 這正是「Model ≠ MLModel」在程式碼層面的證明。

### 2.1 標準 Transform 詞彙（Pipeline 的組成單位）

`python-transform` 提供 22 個已審核的轉換。**22 個全部以 Arrow 實作**
...
| 檔案 | 缺少時的行為 |
|------|--------------|
| `data/typhoon/preprocessed/typhoons_overview.json` | 種子資料略過颱風 Dataset；颱風模型仍會建立，執行時回 404 並說明缺少哪個檔案 |
| `data/typhoon/preprocessed/precip_analog.npz` | 降水模型在 `validate()` 時發出 warning，執行時回 404 |
| `data/samples/sales.csv` | 首次啟動自動生成，不需準備 |
...
以及「Typhoon analog forecast」與「Sales analytics」兩個已發布的內建應用。
