"""
實驗 003：Combined RRF Optimized (最佳化參數)

參數配置：
  - alpha (KNN 權重): 0.10
  - rule_weight (Rule-Based 權重): 0.40
  - w_dtw = 1 - alpha - rule_weight = 0.50
  - rrf_k: 30（最佳化平滑常數）
  - k: 5
  - pool_size_factor: 10
  - dtw_weights: [1.0, 0.5, 1.0, 0.5]（距離與風速權重高）
...
執行：python experiments/typhoon/all_cases/exp003_combined_optimized/run.py
"""
...
# === 實驗配置（完整記錄，供未來重現）===
EXPERIMENT_CONFIG = {
    "name": "exp003_combined_optimized",
    "description": "Combined RRF Optimized: KNN(0.10) + DTW(0.50) + Rule-Based(0.40), rrf_k=30, feature-weighted KNN",
...
def main():
    print("=" * 60)
    print("[EXP] 003: Combined RRF Optimized")
    print("=" * 60)
    ...
    meta = {
        ...
        "config_source": "experiments/typhoon/all_cases/exp003_combined_optimized/run.py",
