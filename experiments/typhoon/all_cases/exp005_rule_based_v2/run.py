"""
實驗 005：Rule-Based Classification v2 (雙層 Window + 向量方向 + 強化分類)

改進內容（相對於 exp002）：
  - 雙層 Impact Window: Core 200km (方向) + Context 400km (趨勢)
  - 向量平均方向取代單一 entry→exit 向量
  - 角度判斷方向 (arctan2) 取代固定閾值
  - 分類優先順序: Cat6 → 特殊 → Cat9 → 登陸 → 海面
  - 登陸判斷三級優先: 文字 → bbox → distance
  - 使用 closest_lat 取代 mean_lat
  - Cat 6 強化條件 (東側比例 + heading + 經度變化)
  - 資料驅動信心分數
...
參數配置：
  ...
  - core_radius_km: 200 (新增)
  - context_radius_km: 400 (新增)
  - weight_path: 0.6
  - weight_category: 0.3
...
執行：python experiments/typhoon/all_cases/exp005_rule_based_v2/run.py
"""
...
# === 實驗配置 ===
EXPERIMENT_CONFIG = {
    "name": "exp005_rule_based_v2",
    "description": "Rule-Based v2: 雙層 Window + 向量方向 + 強化分類優先順序 + 資料驅動信心分數",
    "method": "rule_based",
    "parameters": {
        "k": 5,
        "impact_radius_km": 500.0,
        "core_radius_km": 200.0,
        "context_radius_km": 400.0,
        "weight_path": 0.6,
        "weight_category": 0.3,
        "weight_intensity": 0.1,
    },
...
def main():
    print("=" * 60)
    print("🧪 實驗 005: Rule-Based Classification v2")
    print("=" * 60)
    print(f"  方法: {EXPERIMENT_CONFIG['method']}")
    print(f"  參數: k={EXPERIMENT_CONFIG['parameters']['k']}")
    print(f"  Core Window: {EXPERIMENT_CONFIG['parameters']['core_radius_km']}km")
    print(f"  Context Window: {EXPERIMENT_CONFIG['parameters']['context_radius_km']}km")
    print(
        f"  權重: path={EXPERIMENT_CONFIG['parameters']['weight_path']}, "
        f"category={EXPERIMENT_CONFIG['parameters']['weight_category']}, "
        f"intensity={EXPERIMENT_CONFIG['parameters']['weight_intensity']}"
    )
    print()
    ...
    meta = {
        ...
        "config_source": "experiments/typhoon/all_cases/exp005_rule_based_v2/run.py",
