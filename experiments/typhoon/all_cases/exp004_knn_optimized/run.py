...
參數配置：
  ...
  - feature_weights: [3.0, 2.0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 2.5, 0.5, 0.5]
    (min_distance, closest_lat 權重高; speed 權重高)

評估方式：Leave-One-Out Cross Validation (198 筆, Cat 1-9)
含降水分析

執行：python experiments/typhoon/all_cases/exp004_knn_optimized/run.py
"""
...
# === 實驗配置 ===
EXPERIMENT_CONFIG = {
    "name": "exp004_knn_optimized",
    "description": "KNN Optimized: 3 significant features weighted (min_distance=3.0, closest_lat=2.0, speed=2.5)",
    "method": "knn_optimized",
    "parameters": {
        "k": 5,
...
def main():
    print("=" * 60)
    print("[EXP] 004: KNN Optimized")
    print("=" * 60)
    print(f"  方法: {EXPERIMENT_CONFIG['method']}")
    print(f"  參數: k={EXPERIMENT_CONFIG['parameters']['k']}")
    print(f"  特徵權重: {EXPERIMENT_CONFIG['parameters']['feature_weights']}")
    print()
    ...
    meta = {
        ...
        "config_source": "experiments/typhoon/all_cases/exp004_knn_optimized/run.py",
